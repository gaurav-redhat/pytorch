...
def _setup_standard_sys_libs(
    ...
    if config.is_fbcode():
        if aot_mode and cpp_stdlib == "libc++":
            ...
            stdlib_isystem = (
                cv1 if os.path.isdir(cv1) else libcxx_paths.include_dir
            )
...
def _get_aoti_libcxx_archive_dir() -> str | None:
    ...
    In the ien.lower fbpkg: archives are at <pkg_root>/lib/<arch>/
    ...
    # Check if running inside ien.lower fbpkg
...
def _require_aoti_libcxx_paths() -> _AotiLibcxxPaths:
    ...
    if not archive_dir:
        raise RuntimeError(
            ...
            "libaoti_c++abi.a, or run inside an ien.lower fbpkg."
        )
    ...
    if not include_dir:
        raise RuntimeError(
            ...
            "are missing. The ien.lower fbpkg may be corrupt."
        )
...
def _stage_aoti_libcxx_files(command: list[str], tmp_dir: str) -> list[str]:
    ...
    if archive_dir:
        ...
        if old_library_arg in command:
            ...
            command = [
                f"-L{tmp_dir}" if arg == old_library_arg else arg for arg in command
            ]
    ...
    staged_include_dir = os.path.join(tmp_dir, "aoti_libcxx_include")
    rewritten = [
        _rewrite_uploaded_libcxx_include_arg(arg, include_dir, staged_include_dir)
        for arg in command
    ]
    if rewritten != command:
        shutil.copytree(include_dir, staged_include_dir)
    return rewritten
...
def get_cpp_torch_options(
    ...
    (
        ...
        cpp_compiler, aot_mode, use_relative_path, cpp_stdlib
    )
