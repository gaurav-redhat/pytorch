...
_CAT_EXPAND_BMM_ALLOWED_BACKENDS = frozenset({"ATEN", "TRITON", "CPP", "CUTLASS"})
_CAT_EXPAND_BMM_ALLOWED_DEVICE_TYPES = frozenset({"cpu", "cuda"})
...
def _sequences_are_statically_equal(
...
def _extract_cat_expand_bmm_match(
    ...
    if not all(isinstance(expand_node, torch.fx.Node) for expand_node in expand_nodes):
        return None
    base_nodes = [
        get_arg_value(expand_node, 0, "self") for expand_node in expand_nodes
    ]
...
def _cat_expand_bmm_rank_rejection(
    ...
    if any(tensor_values[node].ndim != 3 for node in match_nodes.expand_nodes):
        return "rank_expand"
    if any(
        tensor_values[node].ndim not in (2, 3) for node in match_nodes.base_nodes
    ):
        return "rank_base"
    return None
...
def _cat_expand_bmm_tensor_values(
    ...
    rejection = _cat_expand_bmm_tensor_property_rejection(
        match_nodes, tensor_values
    )
    if rejection is not None:
        return None, rejection
...
def _cat_expand_bmm_shape_rejection(
    ...
    expand_shapes = [
        tensor_values[node].shape for node in match_nodes.expand_nodes
    ]
...
def _cat_expand_bmm_admission_rejection(
    ...
    if not backends or not backends.issubset(_CAT_EXPAND_BMM_ALLOWED_BACKENDS):
        return "backend"
...
def _cat_expand_bmm_user_rejection(
    ...
    if len(match_nodes.cat_node.users) != 1 or cat_user not in match_nodes.cat_node.users:
        return "users"
    if match_nodes.outer_expand_node is not None:
        if (
    ...
    if len(set(match_nodes.expand_nodes)) != len(match_nodes.expand_nodes):
        return "users"
    nodes_to_erase = {match_nodes.cat_node, *match_nodes.expand_nodes}
    if match_nodes.outer_expand_node is not None:
...
def _cat_expand_bmm_expand_argument_rejection(
    ...
    if any(
        not isinstance(
            size := get_arg_value(expand_node, 1, "size"), (list, tuple)
        )
        or len(size) != 3
...
def _build_cat_expand_bmm_rewrite_plan(
    ...
    if rejection is not None:
        return None, rejection
    rejection = _cat_expand_bmm_shape_rejection(
        match_nodes, cat_dim, tensor_values
    )
    if rejection is not None:
        return None, rejection
...
@register_graph_pattern(
    CallFunction(
        aten.bmm.default, _identity_expand_batch_broadcast_cat, Ignored()
    ),
    # pyrefly: ignore [bad-argument-type]
...
def cat_expand_to_batch_stride_zero(match: Match) -> None:
    ...
    base_values = [
        rewrite_plan.tensor_values[base_node]
        for base_node in match_nodes.base_nodes
    ]
    with match.graph.inserting_before(match_nodes.cat_node):
        ...
        inserted_unsqueeze_nodes: list[torch.fx.Node] = []
        for base_node, base_value in zip(
            ...
            if base_value.ndim == 2:
                ...
                inserted_unsqueeze_nodes.append(base_node)
