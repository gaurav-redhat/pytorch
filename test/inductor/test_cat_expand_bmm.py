...
class CatExpandBmmTest(TestCase):
    ...
    def test_rewrite(self, cat_dim, outer_expand, direct_rank3):
        ...
        def record_bmm_lhs_stride(graph):
            for node in graph.find_nodes(
                op="call_function", target=aten.bmm.default
            ):
    ...
    def test_production_topology(self):
        ...
        with inductor_config.patch(
            ...
            second_rhs = torch.randn(
                3, 1890, 256, device="cpu", dtype=torch.float16
            )
    ...
    def test_production_topology_rank2_base(self):
        ...
        weights = [
            torch.randn(10, 1890, device="cpu", dtype=torch.float16)
            for _ in range(15)
        ]
        rhs = torch.randn(2, 1890, 256, device="cpu", dtype=torch.float16)
        graph_metadata = []
        ...
        self.assertEqual(
            metadata["outer_expand"], ((150, 1890), (283500, 1890, 1))
        )
        self.assertEqual(
            metadata["rhs"],
    ...
    def test_rewrite_disabled(self):
        ...
        weights = [
            torch.randn(rows, 8, device="cpu", dtype=torch.float16)
            for rows in (2, 3)
        ]
        rhs = torch.randn(4, 8, 6, device="cpu", dtype=torch.float16)
        lhs_strides = []
    ...
    def test_rewrite_rejected_rhs_alias(self, outer_expand):
        def fn(weights):
            lhs = torch.cat(
                [
                    weight.unsqueeze(0).expand(3, -1, -1)
                    for weight in weights
                ],
                dim=1,
            )
        ...
        weights = [
            torch.randn(2, 4, device="cpu", dtype=torch.float16)
            for _ in range(2)
        ]
    ...
    def test_rewrite_rejected(self, case):
        ...
        if case == "batch_cat":
            weights = [
                torch.randn(2, 5, device="cpu", dtype=torch.float16)
                for _ in range(2)
            ]
            ...
            weights = [
                torch.randn(1, 5, device="cpu", dtype=torch.float16)
                for _ in range(2)
            ]
            ...
            weights = [
                torch.randn(3, 5, device="cpu", dtype=torch.float16)
                for _ in range(2)
            ]
            rhs = torch.randn(3, 5, 4, device="cpu", dtype=torch.float16)
    ...
    def test_rewrite_rejected_metadata(self, metadata_case):
        ...
        weights = [
            torch.randn(1, 2, 5, device="cpu", dtype=torch.float16)
            for _ in range(2)
        ]
        rhs = torch.randn(3, 5, 4, device="cpu", dtype=torch.float16)
        saved_args = {}
